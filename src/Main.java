...
public class Main{
    public static void main(String[]args){
        ...
        if(mode==0){
            System.out.print("Choose between 0 (Rock), 1 (Paper) and 2 (Scissor): ");int player=input.nextInt();
            System.out.println(Rps.convertResult(Game.game(player)));
        }else if(mode==1){
            Games games=new Games();int i=0;
            while(i<25){
                System.out.print("Choose between 0 (Rock), 1 (Paper) and 2 (Scissor): ");
                int player=input.nextInt();int result=Game.game(player);
                System.out.println(Rps.convertResult(result));
                if(result!=-1){games.addResult(result);i++;}
            }
            System.out.println();
            ...
            Games games=new Games();games.setMode(1);
            int cpuOne=0,i=0;
            while(i<100){cpuOne=(int)(Math.random()*3);int result=Game.game(cpuOne);if(result!=-1){games.addResult(result);i++;}}
            System.out.println();
            System.out.println("Number of Draws: "+games.numbersOfDraws());
        ...
        }else{System.out.println("No valid mode was selected");}
    }
}